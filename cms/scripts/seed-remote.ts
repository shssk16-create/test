...
const heroes = [
  { id: "hero-salmeen-uuid", name_ar: "سالمين هادي.", name_en: "Salmeen Hadi.", title_ar: "مدير منتجات ذكاء اصطناعي", title_en: "AI Product Manager", subtitle_ar: "أبني منصات رقمية متكاملة. يمكنك تصفح معرض أعمالي من الأعلى، أو التحدث مباشرة مع مساعدي الذكي ليجيب على كافة استفساراتك حول خبراتي وتقنياتي.", subtitle_en: "Building comprehensive digital platforms. Explore my portfolio from the menu above or chat with my custom-built AI assistant for any inquiries.", whatsapp_number: "966503026795" }
];

const logos = [
  { id: "logo-1", name: "Aura Marketing", imageUrl: "https://aurateam3.com/wp-content/uploads/2024/02/cropped-%D8%B4%D8%B9%D8%A7%D8%B1-%D8%A3%D9%88%D8%B1%D8%A7-02-2.png", sort_order: "1" },
  { id: "logo-2", name: "Asas Holding", imageUrl: "https://floralwhite-dove-225940.hostingersite.com/wp-content/uploads/2025/12/%D8%A3%D8%B3%D8%A7%D8%B3-1.webp", sort_order: "2" },
  { id: "logo-3", name: "Akam Contracting", imageUrl: "https://akamcont.sa/wp-content/uploads/2025/11/cropped-1000034239.webp", sort_order: "3" },
  { id: "logo-4", name: "Redp SA", imageUrl: "https://redp-sa.com/web/images/logo.svg", sort_order: "4" },
  { id: "logo-5", name: "Bathq SA", imageUrl: "https://bathq.sa/wp-content/uploads/2025/11/1000033762.webp", sort_order: "5" },
  { id: "logo-6", name: "Darb Stations", imageUrl: "https://darbstations.com.sa/wp-content/uploads/2024/12/1221-copy-3.png", sort_order: "6" }
];

const sqlStatements: string[] = [];

for (const p of projects) {
  sqlStatements.push(`INSERT OR REPLACE INTO projects (id, title_ar, title_en, subtitle_ar, subtitle_en, accentColor, thumbIcon, problem_ar, problem_en, decision_ar, decision_en, result_ar, result_en, stack, year, featured, logo, image, link) VALUES ('${p.id}', '${p.title_ar.replace(/'/g, "''")}', '${p.title_en.replace(/'/g, "''")}', '${p.subtitle_ar.replace(/'/g, "''")}', '${p.subtitle_en.replace(/'/g, "''")}', '${p.accentColor}', '${p.thumbIcon}', '${p.problem_ar.replace(/'/g, "''")}', '${p.problem_en.replace(/'/g, "''")}', '${p.decision_ar.replace(/'/g, "''")}', '${p.decision_en.replace(/'/g, "''")}', '${p.result_ar.replace(/'/g, "''")}', '${p.result_en.replace(/'/g, "''")}', '${p.stack.replace(/'/g, "''")}', '${p.year}', ${p.featured}, '${p.logo}', '${p.image}', '${p.link}');`);
}
...
for (const c of certificates) {
  ...
  sqlStatements.push(`INSERT OR REPLACE INTO certificates (id, name_ar, name_en, issuer_ar, issuer_en, date, credential_url, image, featured, skills, owner, description_ar, description_en, degree_level_ar, degree_level_en) VALUES ('${c.id}', '${c.name_ar.replace(/'/g, "''")}', '${c.name_en.replace(/'/g, "''")}', '${c.issuer_ar.replace(/'/g, "''")}', '${c.issuer_en.replace(/'/g, "''")}', '${c.date}', '${c.credential_url}', '${c.image}', ${c.featured}, ${skillsVal}, ${ownerVal}, ${descArVal}, ${descEnVal}, ${degArVal}, ${degEnVal});`);
}

for (const h of heroes) {
  sqlStatements.push(`INSERT OR REPLACE INTO heroes (id, name_ar, name_en, title_ar, title_en, subtitle_ar, subtitle_en, whatsapp_number) VALUES ('${h.id}', '${h.name_ar.replace(/'/g, "''")}', '${h.name_en.replace(/'/g, "''")}', '${h.title_ar.replace(/'/g, "''")}', '${h.title_en.replace(/'/g, "''")}', '${h.subtitle_ar.replace(/'/g, "''")}', '${h.subtitle_en.replace(/'/g, "''")}', '${h.whatsapp_number}');`);
}

for (const l of logos) {
  sqlStatements.push(`INSERT OR REPLACE INTO logos (id, name, imageUrl, sort_order) VALUES ('${l.id}', '${l.name.replace(/'/g, "''")}', '${l.imageUrl}', '${l.sort_order}');`);
}

const sqlContent = sqlStatements.join('\n');
const tempSqlPath = pathNode.join(__dirname, 'seed-remote.sql');

try {
  fs.writeFileSync(tempSqlPath, sqlContent, 'utf8');
  console.log('Seeding REMOTE D1 database...');
  
  execSync(`npx wrangler d1 execute cms-db --remote --file=scripts/seed-remote.sql`, {
    cwd: pathNode.join(__dirname, '..'),
    stdio: 'inherit'
  });
  
  console.log('✅ Remote D1 database seeded successfully!');
} catch (err: any) {
