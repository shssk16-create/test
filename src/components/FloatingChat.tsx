...
export default function FloatingChat({ owner = 'salmeen' }: { owner?: string }) {
  ...
  const isMainPage = true; // Summon interactive AI Assistant
  ...
  return (
    <div className={`keep-rounds fixed bottom-4 sm:bottom-6 ${isAr ? 'right-3 sm:right-6' : 'left-3 sm:left-6'} z-[100] ${isAr ? 'font-alexandria' : 'font-sans'} flex flex-col ${isAr ? 'items-end' : 'items-start'} max-w-[calc(100vw-24px)]`} dir={isAr ? 'rtl' : 'ltr'}>
      <AnimatePresence>
        {isOpen && (
            ...
            <div className={`${isDark ? (isAmal ? 'bg-[#25303c] border-[#547A95]/20' : 'bg-[#111318] border-[#1e2330]') : (isAmal ? 'bg-white border-[#547A95]/20' : 'bg-white border-stone-200')} p-4 flex justify-between items-center border-b`}>
              ...
              <div className="flex items-center gap-2">
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className={`w-8 h-8 rounded-full ${isDark ? 'bg-white/10 text-green-400 hover:bg-white/20' : 'bg-stone-100 text-green-600 hover:bg-stone-200'} flex items-center justify-center transition-all`}
